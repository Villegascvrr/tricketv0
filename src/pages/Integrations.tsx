...
const Integrations = () => {
  ...
  // Ticketing Data
  const demoTicketing = [
    { name: "Ticketmaster", connected: true, events: 8, lastSync: "Hace 2 horas", tickets: 12500 },
    { name: "Entradas.com", connected: true, events: 5, lastSync: "Hace 4 horas", tickets: 8200 },
    { name: "Bclever", connected: true, events: 3, lastSync: "Hace 1 hora", tickets: 3400 },
    { name: "Forvenues", connected: false, events: 0, lastSync: null, tickets: 0, pending: true },
  ];

  const realTicketing = [
    { name: "Ticketmaster", connected: false, events: 0, lastSync: null, tickets: 0 },
    { name: "Entradas.com", connected: false, events: 0, lastSync: null, tickets: 0 },
    { name: "Bclever", connected: false, events: 0, lastSync: null, tickets: 0 },
    { name: "Forvenues", connected: false, events: 0, lastSync: null, tickets: 0 },
  ];

  const ticketingProviders = isDemo ? demoTicketing : realTicketing;

  // Marketing Data
  const demoMarketing = [
    { name: "Meta Ads", status: "pending", connected: false },
    { name: "Google Ads", status: "pending", connected: false },
    { name: "TikTok Ads", status: "planned", connected: false },
    { name: "Twitter Ads", status: "planned", connected: false }
  ];

  // For integrations, we treat "Pending" as the default for real events too until implemented
  // So we can reuse the structure but ensure "connected" is false and counts are 0 if we were showing counts

  // Email Data
  const demoEmail = [
    { name: "Mailchimp", connected: true, campaigns: 5, subscribers: 32000, lastSync: "Hace 2h" },
    { name: "Brevo", connected: false, campaigns: 0, subscribers: 0, lastSync: null }
  ];

  const realEmail = [
    { name: "Mailchimp", connected: false, campaigns: 0, subscribers: 0, lastSync: null },
    { name: "Brevo", connected: false, campaigns: 0, subscribers: 0, lastSync: null }
  ];

  const emailProviders = isDemo ? demoEmail : realEmail;
      ...
      <div className="max-w-7xl mx-auto space-y-4">
        ...
        <div className="mb-1">
          ...
          <p className="text-xs text-muted-foreground">
            Conexiones configuradas para {eventName}
          </p>
        </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  ...
                  son configuradas y mantenidas por el equipo de Tricket.
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            {ticketingProviders.map((provider, i) => (
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {provider.connected ? (
                      ...
                      <Badge variant="outline" className="text-[10px] px-1.5 py-0 gap-1">
                        ...
                        {isDemo ? "En proceso" : "No conectado"}
                      </Badge>
                    )}
                  </div>
                  ...
                  <div className="pt-1">
                    {provider.lastSync ? (
                      ...
                      <span className="text-[10px] text-muted-foreground">
                        {isDemo ? "Configuración por Tricket en proceso" : "Requiere configuración"}
                      </span>
                    )}
                  </div>
                </CardContent>
              ...
              <Card key={i} className="opacity-70">
                <CardContent className="pt-4 space-y-3">
                  ...
                  <div className="flex items-center justify-between">
                    <Badge variant="outline" className="text-[10px] px-1.5 py-0 gap-1">
                      <Clock className="h-2.5 w-2.5" />
                      {platform.status === 'pending' ? (isDemo ? 'En configuración' : 'No conectado') : (isDemo ? 'Próximamente' : 'No disponible')}
                    </Badge>
                  </div>
                </CardContent>
              </Card>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
            {emailProviders.map((platform, i) => (
              <Card key={i} className={!platform.connected ? 'opacity-60' : ''}>
                <CardContent className="pt-4 space-y-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      <div className={`p-1.5 rounded-lg ${platform.name === 'Mailchimp' ? "text-yellow-600 bg-yellow-500/10" : "text-indigo-500 bg-indigo-500/10"}`}>
                      ...
                      <div>
                        {/* We use isDemo for description logic if needed, but here simple mapping is fine */}
                        <span className="text-sm font-medium block">{platform.name}</span>
                        <span className="text-[10px] text-muted-foreground">{platform.name === 'Mailchimp' ? "Email automation" : "Email + SMS"}</span>
                      </div>
                    </div>
                    {platform.connected ? (
                      <Badge className="bg-success/10 text-success border-success/20 text-[10px] px-1.5 py-0 gap-1">
            ...
            <Card className="opacity-60">
              <CardContent className="pt-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant="outline" className="text-[10px] px-1.5 py-0 gap-1">
                    <Clock className="h-2.5 w-2.5" />
                    {isDemo ? "En proceso" : "No disponible"}
                  </Badge>
                </div>
